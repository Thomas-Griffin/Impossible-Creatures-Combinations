<template>
  <div class="flex justify-center flex-center">
    <q-btn-group>
      <q-btn icon="add" label="Add Chart" outline @click="newChart()" />
    </q-btn-group>
  </div>
  <div v-for="chart in charts" :key="chart.id" class="flex justify-center flex-center q-pa-sm">
    <q-btn :key="chart.id" :value="chart.id" icon="delete" label="Delete" outline @click="removeChart(chart.id)" />
