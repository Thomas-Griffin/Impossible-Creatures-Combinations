<template>
    <div class="h-full w-full justify-content-center align-items-center">
        <div class="row">
            ...
            <InputNumber v-model="bucketSize" :min="1" @update:model-value="updateChart" />
        </div>
        <div :id="chartId">
            <NuxtPlotly :config="config" :data="chartData" :layout="layout" style="width: 100%" />
        </div>
    </div>
</template>
...
const axisOptions = ref(Object.values(CombinationVisualisationAttributes))

const xAttribute = ref<CombinationVisualisationAttributes>(CombinationVisualisationAttributes.NONE)
const yAttribute = ref<CombinationVisualisationAttributes>(CombinationVisualisationAttributes.NONE)
...
const chartType = computed(() => {
    if (yAttribute.value === CombinationVisualisationAttributes.NONE) {
        return CombinationVisualisationTypes.ATTRIBUTE
    }
    return CombinationVisualisationTypes.X_PER_Y
})

const fetchFunction = computed(() => {
    switch (chartType.value) {
        case CombinationVisualisationTypes.ATTRIBUTE:
            return getAttributeChart
        case CombinationVisualisationTypes.X_PER_Y:
...
const layout = computed(
    () =>
        ({
            ...
            title:
                yAttribute.value === CombinationVisualisationAttributes.NONE
                    ? xAttribute.value
